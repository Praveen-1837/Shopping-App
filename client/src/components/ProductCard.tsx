    ...
    <div className="bg-background-card rounded-2xl border border-text-muted/15 overflow-hidden shadow-soft hover:shadow-card hover:-translate-y-0.5 transition-all duration-200 flex flex-col justify-between group relative">
      <div>
        ...
        <div className="relative aspect-[4/3] bg-background-muted overflow-hidden">
          ...
          {/* Category Pill */}
          <div className="absolute top-2.5 left-2.5 bg-background-card/90 backdrop-blur-md px-2 py-0.5 rounded-full text-[11px] font-semibold text-primary border border-text-muted/10 shadow-xs">
            {product.category}
          </div>
          ...
          <button
            ...
            className={`absolute bottom-2.5 right-2.5 p-1.5 rounded-full backdrop-blur-md transition-all shadow-xs cursor-pointer ${
              isWishlisted
        ...
        <div className="p-3.5 space-y-2">
          ...
          {/* Title */}
          <h3 className="font-heading font-bold text-sm text-text-primary group-hover:text-primary transition-colors line-clamp-2 leading-snug">
          ...
          {product.sustainabilityTags?.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-0.5">
              {product.sustainabilityTags.slice(0, 2).map((tag, idx) => (
                <span
                  key={idx}
                  className="inline-flex items-center space-x-0.5 bg-primary-light text-primary text-[9px] px-1.5 py-0.5 rounded-md font-semibold"
                >
      ...
      <div className="p-3.5 pt-2 flex items-center justify-between border-t border-text-muted/10 mt-1 bg-background-muted/30">
        <div>
          <span className="text-[10px] text-text-muted block uppercase tracking-wider font-semibold">Price</span>
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-xl font-bold text-xs transition-all shadow-soft cursor-pointer flex items-center space-x-1.5 ${
            added
