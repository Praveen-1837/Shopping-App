    ...
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-text-muted/15 pb-6">
        ...
        <Link
          ...
          className="inline-flex items-center space-x-2 px-5 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-hover transition-colors shadow-soft cursor-pointer self-start sm:self-auto"
        >
      ...
      {isLoading ? (
        <div className="py-20 text-center text-text-secondary space-y-3">
          ...
          <p className="text-base font-medium">Fetching your product catalog...</p>
        </div>
      ) : isError ? (
        <div className="bg-error-light border border-error/30 rounded-2xl p-8 text-center space-y-4">
          ...
          <button
            onClick={() => refetch()}
            className="px-4 py-2 bg-error text-white text-sm font-semibold rounded-lg hover:bg-error/90"
          >
            Retry
          </button>
        </div>
        ...
        <div className="bg-background-card rounded-2xl p-12 text-center border border-text-muted/15 space-y-4">
          ...
          <Link
            to="/seller/products/new"
            className="inline-flex items-center space-x-2 px-5 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-hover transition-colors shadow-soft"
          >
            <Plus className="w-4 h-4" />
        ...
        <div className="bg-background-card rounded-2xl border border-text-muted/15 overflow-hidden shadow-soft">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-background-muted/70 text-text-secondary text-sm uppercase font-semibold border-b border-text-muted/15">
              ...
              <tbody className="divide-y divide-text-muted/10 text-base">
                {products.map((prod) => (
                        ...
                        {deleteTargetId === prod.id ? (
                          <div className="flex items-center space-x-1">
                            <button
                              ...
                              className="px-2.5 py-1 bg-error text-white text-sm font-bold rounded-lg"
                            >
                              Confirm
                            </button>
                            <button
                              onClick={() => setDeleteTargetId(null)}
                              className="px-2 py-1 bg-background-muted text-sm rounded-lg"
                            >
                              Cancel
                            </button>
                          </div>
