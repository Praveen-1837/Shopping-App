    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="bg-background-card rounded-3xl p-6 border border-text-muted/15 shadow-soft">
        {isLoading ? (
          ...
          <p className="text-error text-sm text-center py-8">Failed to load requests.</p>
        ) : requests.length === 0 ? (
          <p className="text-text-muted text-sm text-center py-12">No pending cancellation requests.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-text-muted/15 text-xs text-text-muted uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-text-muted/10 text-xs">
                  ...
                  <tr key={req.id} className="hover:bg-background-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right space-x-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-error/10 hover:bg-error/20 text-error text-[11px] font-bold rounded-lg transition-colors inline-flex items-center space-x-1 cursor-pointer disabled:opacity-50"
                      >
