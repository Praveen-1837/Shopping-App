...
import { 
  ...
  ChevronRight,
  Filter,
  Sprout, PlusCircle, Package, IndianRupee, Truck, Trash2, Edit, Video, Eye, X, ArrowUp, ArrowDown, Image as ImageIcon
} from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <form onSubmit={handleSubmitProduct(onSubmitProduct)} className="space-y-6">
              ...
              <div className="flex gap-3 justify-end pt-6 border-t border-border-muted mt-6">
                <button type="button" onClick={() => setShowAddProductModal(false)} className="px-6 py-3 font-bold text-text-primary bg-gray-100 hover:bg-gray-200 rounded-xl transition">
                  Cancel
                </button>
                <button type="submit" disabled={isUploadingImage || isSubmittingProduct || createProductMutation.isPending} className="px-8 py-3 font-bold text-white bg-primary hover:bg-primary-hover disabled:opacity-70 disabled:cursor-not-allowed rounded-xl transition flex items-center gap-2">
                  {isUploadingImage || createProductMutation.isPending ? (
      ...
      <div className="flex gap-2 border-b border-border-muted overflow-x-auto pb-px scrollbar-hide">
        <button onClick={() => setActiveTab('dashboard')} className={\`px-6 py-4 font-bold whitespace-nowrap border-b-2 transition \${activeTab === 'dashboard' ? 'border-primary text-primary' : 'border-transparent text-text-muted hover:text-text-primary'}\`}>
          Dashboard
        </button>
        <button onClick={() => setActiveTab('add-products')} className={\`px-6 py-4 font-bold whitespace-nowrap border-b-2 transition \${activeTab === 'add-products' ? 'border-primary text-primary' : 'border-transparent text-text-muted hover:text-text-primary'}\`}>
          Products
        </button>
        <button onClick={() => setActiveTab('fulfillment')} className={\`px-6 py-4 font-bold whitespace-nowrap border-b-2 transition \${activeTab === 'fulfillment' ? 'border-primary text-primary' : 'border-transparent text-text-muted hover:text-text-primary'}\`}>
          Fulfillment Orders
        </button>
      </div>
        ...
        <div className="space-y-6 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-background-card border-2 border-secondary/20 rounded-3xl p-8 shadow-soft space-y-3">
              ...
              <div>
                <span className="text-sm font-bold text-text-muted uppercase tracking-wider block">Total Farm Revenue</span>
                <span className="text-3xl font-black font-heading text-secondary">₹{analyticsData.totalRevenue.toLocaleString('en-IN')}</span>
              </div>
            </div>
            ...
            <div className="bg-background-card border-2 border-primary/20 rounded-3xl p-8 shadow-soft space-y-3">
              ...
              <div>
                <span className="text-sm font-bold text-text-muted uppercase tracking-wider block">Harvest Produce Listed</span>
                <span className="text-3xl font-black font-heading text-primary">{analyticsData.totalProducts} Items</span>
              </div>
            </div>
                ...
                {productsData.pagination?.pages > 1 && (
                  <div className="flex flex-col sm:flex-row items-center justify-between p-6 border-t border-border-muted bg-gray-50/50">
                    ...
                    <div className="flex gap-2 mt-4 sm:mt-0">
                      <button onClick={handleProductsPreviousPage} disabled={productsPage === 1} className="flex items-center gap-1 px-4 py-2 text-sm font-bold bg-white border border-border-muted rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:hover:bg-white transition shadow-sm">
                        <ChevronLeft className="w-4 h-4" /> Prev
                      </button>
                      <button onClick={handleProductsNextPage} disabled={!productsData.pagination.hasMore} className="flex items-center gap-1 px-4 py-2 text-sm font-bold bg-white border border-border-muted rounded-xl hover:bg-gray-50 disabled:opacity-50 disabled:hover:bg-white transition shadow-sm">
      ...
      {activeTab === 'fulfillment' && (
        <div className="space-y-6 pt-4">
          ...
          {ordersLoading && <div className="text-center py-16"><Loader2 className="w-10 h-10 animate-spin mx-auto text-primary" /></div>}
        </div>
      )}
    </div>
  );
...
console.log("Successfully rewrote FarmerCentre.tsx to support multiple images");
